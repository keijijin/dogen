...
- 問答 API に `POST /api/v1/chat/stream`（SSE）を追加し、応答を逐次配信
- 問答ドック（`web/js/chat-dock.js`）をストリーミング受信 + 失敗時 `/api/v1/chat` フォールバックに対応
- 問答ドックの assistant 応答を安全な範囲で Markdown 表示（太字・コード・リンク・コードブロック）
- OpenShift Postgres のローカルバックアップ/リストア運用を追加（毎日 6:30 テンプレート、7日保持）
