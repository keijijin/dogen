...
| 補助イラスト | 五段階の直後に `img/正法眼蔵.png`（書物・学びのイメージ）。`<figure class="home-illustration">` で表紙よりやや小さい最大幅に抑え、主従を分ける。 |
| 構成 | 目的（ヒーロー）→ タイトル画 → **読み方の五段階**（`.home-steps`）→ 補助イラスト → 学習総覧＋コース → 入り口カード → 優先巻リスト → 概念マンガ。スタイルは `css/main.css` の `.home-cover` / `.home-steps` / `.home-illustration`。 |
...
| 日付 | 内容 |
|------|------|
| 2026-04-28 | 初版。トップ再編（`表紙.png` タイトル画、五段階直後に `正法眼蔵.png` 補助イラスト）。 |
