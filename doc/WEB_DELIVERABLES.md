...
| 構成 | 目的（ヒーロー）→ タイトル画 → **読み方の五段階**（`.home-steps`）→ 学習総覧＋コース → 入り口カード → 優先巻リスト → 概念マンガ。スタイルは `css/main.css` の `.home-cover` / `.home-steps`。 |
...
| 日付 | 内容 |
|------|------|
| 2026-04-28 | 初版。同日: トップを教育的な順路に再編し `表紙.png` をタイトル画として掲載。 |
